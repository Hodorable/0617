...
class PolicyUI(BaseModel):
    ...
    objects = sa.Column(sa.Text(), nullable=False)
    vi_condition = sa.Column(sa.Text(), nullable=False)
    actions = sa.Column(sa.Text(), nullable=False)
...
def add_data(id_, name_tt, objects_tt, vi_condition_tt, actions_tt,
           data_tt,deleted=False, session=None):
    session =session or Session()
    #with session.begin(subsactions=True):
    try:
      lt = PolicyUI(id_, name_tt, objects_tt, vi_condition_tt,actions_tt,data_tt)
      session.add(lt)
      session.commit()
      return 0
    except Exception ,e:
